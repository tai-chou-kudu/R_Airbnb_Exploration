...
This isn't the best use of space, as Bed-Stuy shows an outlier of maximum price in its histogram chart. A future idea is to normalize the data or transform to account for the outlier, so that we can see more of the data for each neighborhood, instead of it looking vertically compressed or barely visible.
